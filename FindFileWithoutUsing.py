...
def find_un_used(array):
    ...
    for i in range(0, len(array)):
        ...
        # strory_board_string = 'instantiateViewControllerWithIdentifier: @"%s"' % file_name  # storyBoard 创建的控制器
        # xib_cell = 'dequeueReusableCellWithIdentifier: @"%s"' % file_name  # storyBoard或xib创建的cell
        string = '[%s' % file_name  # 创建代码，例如： [[WLJYJViewController alloc] init];
        extension_class = '(%s)' % file_name  # 分类
        strory_board_string = 'instantiateViewControllerWithIdentifier:'  # storyBoard 创建的控制器
        xib_cell = 'dequeueReusableCellWithIdentifier:'  # storyBoard或xib创建的cell
        story_board_view_str = '@property (weak, nonatomic) IBOutlet %s *' % file_name  # storyBoard 创建的view
        # print("分类数据",extensionClass)
        # thirdTool = 'https://github.com/*/%s' % file_name  # 第三方 好像不对
        # re.match(thirdTool, file_name)
        if (file_name in result) and ('https://github.com' in result):
            print("是否是第三方：是")
        else:
            print('是否是第三方：否')
        ...
        elif (string in result) or (extension_class in result) \
                or ((file_name in result) and ('https://github.com' in result)) \
                or (story_board_view_str in result) or (xib_cell in result) or (strory_board_string in result):
            # and ('YYGestureRecognizer' == file_name) and ('YYC' not in file_name[0:3]) and ('YYText' not in
            # file_name[0:6]) and ('YYThread' not in file_name[0:8])
            print("有引用")
...
if __name__ == '__main__':
    ...
    print("\n 未使用的文件个数：", len(unused_files), "\n 可能没有引用的文件是：", unused_files)

    # 将确定使用的类存放在数组中进行过滤
    array = ['FMFunctions', 'FMGoodShopHeaderModel', 'FMIMBaseTableVIewCell', 'FMIMInputContentView', 'FMShopSpecModel',
             'FMViewController', 'FSCalendarConstants',
             'LLLockViewController', 'LRLAVPlayerTool', 'LRLLightView', 'LTSCalendarContentView',
             'LTSCalendarManager', 'LTSCalendarSelectedWeekView', 'LTSCalendarWeekDayView', 'LWImageBrowserButton',
             'LWLayout', 'NSDataEx', 'RFImageToDataTransformer', 'TADotView',
             'TURNSocket', 'TimeSheetView', 'JBNetSwiftMangerOC']
